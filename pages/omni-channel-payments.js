...
export default function OmniChannelPayments({ data }) {
    const {
        name,
        image,
        description,
    ...
    return (
        <>
            ...
            <Main>
                ...
                <section id="omniPaymentsBullets">
                    <h2>{bulletsHeader}</h2>
                    <ProductsServicesBullets productFeatures={bullets} />
                </section>

                <section id="omniPaymentsFeatures">
                    {displayFeatures}
                </section>
...
export async function getStaticProps() {
    const data = await import('./api/pages/omni-channel-payments.json');
