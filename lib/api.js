async function fetchAPI(query, { variables } = {}) {
  ...
  export async function getRecentThreePosts() {
    const data = await fetchAPI(
      `
      query AllPosts {
        posts(first: 3, where: { categoryName: "news", orderby: { field: DATE, order: DESC } }) {
          edges {
            node {
              title
              excerpt
  ...
  //Trading View Widget
  export async function getTradingViewWidget() {
    const data = await fetchAPI(
      `
        query MyQuery {
          pages(where: {title: "Trading View Widget"}) {
            edges {
              node {
                content
              }
